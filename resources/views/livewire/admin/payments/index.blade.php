<div class="page-enter space-y-6">
    ...
    @if ($activePaymentId && $activePayment)
        ...
        <div class="modal-backdrop-enter fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 sm:p-6">
            <div class="modal-content-enter flex w-full max-w-3xl flex-col max-h-full overflow-hidden rounded-2xl bg-white shadow-xl">
                <div class="flex flex-shrink-0 items-center justify-between border-b border-slate-100 p-5">
                    <div>
                    ...
                    <button
                        ...
                        class="rounded-lg border border-slate-200 px-3 py-1 text-sm font-semibold text-slate-600 transition hover:bg-slate-50"
                    >
                        Tutup
                    </button>
                </div>
                ...
                <div class="flex-1 overflow-y-auto p-5">
                    @if ($proofPath)
                        @if ($isImage)
                            <div class="flex items-center justify-center rounded-xl bg-slate-50 p-2 sm:p-4 border border-slate-100">
                                <img
                                    src="{{ route('payments.proof', $activePayment) }}"
                                    alt="Bukti"
                                    class="max-h-[50vh] w-auto rounded-lg object-contain shadow-sm"
                                />
                            </div>
                        @else
                ...
                <div class="flex flex-shrink-0 flex-col gap-3 border-t border-slate-100 bg-slate-50 p-5 sm:flex-row sm:items-center sm:justify-between rounded-b-2xl">
                    <div class="text-sm font-medium text-slate-500">
                        Total Pembayaran: <span class="font-bold text-slate-900 text-lg">Rp {{ number_format($activePayment->amount, 0, ',', '.') }}</span>
                    </div>
