<div class="page-enter space-y-6">
    <div>
        ...
        <p class="text-sm text-slate-500 dark:text-stone-400">Kontrol pesanan untuk customer {{ $order->customer->name }}.</p>
    </div>

    <div class="flex flex-wrap items-center gap-2">
        <div class="text-lg font-semibold text-slate-900 dark:text-stone-100">{{ $order->order_number }}</div>
    ...
    <div class="grid gap-6 lg:grid-cols-3">
        <div class="space-y-6 lg:col-span-2">
            <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-stone-700 dark:bg-stone-800">
                <h2 class="text-lg font-semibold">Informasi Pesanan</h2>
                <div class="mt-4 grid gap-4 sm:grid-cols-2">
                    <div>
                        <div class="text-sm text-slate-500 dark:text-stone-400">Customer</div>
                        <div class="text-base font-semibold">{{ $order->customer->name }}</div>
                        <div class="text-sm text-slate-500 dark:text-stone-400">{{ $order->customer->email }}</div>
                    </div>
                    <div>
                        <div class="text-sm text-slate-500 dark:text-stone-400">Layanan</div>
                        <div class="text-base font-semibold">{{ $order->service->name }}</div>
                        <div class="text-sm text-slate-500 dark:text-stone-400">{{ strtoupper($order->service->type) }}</div>
                    </div>
                    <div>
                        <div class="text-sm text-slate-500 dark:text-stone-400">Estimasi Selesai</div>
                        <div class="text-base font-semibold">
                            {{ $order->estimated_finish_date ? $order->estimated_finish_date->format('d M Y') : '-' }}
                        </div>
                    </div>
                    <div>
                        <div class="text-sm text-slate-500 dark:text-stone-400">Estimasi Harga</div>
                        <div class="text-base font-semibold">
                    ...
                    <div>
                        <div class="text-sm text-slate-500 dark:text-stone-400">DP (Ditetapkan Admin)</div>
                    ...
                    @endif
                    <div>
                        <div class="text-sm text-slate-500 dark:text-stone-400">Appointment</div>
                        <div class="text-base font-semibold">
                    ...
                    <div>
                        <div class="text-sm text-slate-500 dark:text-stone-400">Jumlah Item</div>
                        <div class="text-base font-semibold">{{ $order->quantity }}</div>
                    </div>
                </div>
                @if ($order->notes)
                    <div class="mt-4 text-sm text-slate-600 dark:text-stone-400">
                        <div class="font-semibold text-slate-900 dark:text-stone-100">Catatan Customer</div>
                        <p class="mt-1">{{ $order->notes }}</p>
                    </div>
                @endif
                
                @if ($order->service->type === 'vermak')
                    <div class="mt-4 rounded-lg border border-slate-200 bg-slate-50 p-4 dark:border-stone-700 dark:bg-stone-700/30">
                        <div class="text-sm font-semibold text-slate-900 dark:text-stone-100 mb-2">Rincian Vermak</div>
                        @php
                            $vermakDetails = json_decode($order->alteration_details, true) ?? [];
                        @endphp
                        @if (count($vermakDetails) > 0)
                            <ul class="space-y-1 text-sm text-slate-700 dark:text-stone-300">
                ...
                @else
                    <div class="mt-4 rounded-lg border border-slate-200 bg-slate-50 p-4 dark:border-stone-700 dark:bg-stone-700/30">
                        <div class="text-sm font-semibold text-slate-900 dark:text-stone-100">Informasi Bahan</div>
                        <div class="mt-2 grid gap-2 sm:grid-cols-2 text-sm">
                            <div>
                                <span class="text-slate-500 dark:text-stone-400">Sumber Bahan:</span>
                                <span class="font-medium text-slate-900 dark:text-stone-100">
                                    {{ $order->material_source === 'customer' ? 'Bawa Sendiri' : ($order->material_source === 'jasa' ? 'Beli di Penjahit' : 'Belum ditentukan') }}
                                </span>
                            </div>
                            <div>
                                <span class="text-slate-500 dark:text-stone-400">Status Bahan:</span>
                                <span class="font-medium text-slate-900 dark:text-stone-100">
                        ...
                        @if ($order->fabric)
                            <div class="mt-3 border-t border-slate-200 pt-3 dark:border-stone-700">
                                <div class="text-xs font-semibold text-slate-500 dark:text-stone-400 mb-2">DETAIL KAIN:</div>
                                <div class="grid gap-2 sm:grid-cols-2 text-sm">
                                    <div>
                                        <span class="text-slate-500 dark:text-stone-400">Nama:</span>
                                        <span class="font-medium text-slate-900 dark:text-stone-100">{{ $order->fabric->name }}</span>
                                    </div>
                                    <div>
                                        <span class="text-slate-500 dark:text-stone-400">Warna:</span>
                                        <span class="font-medium text-slate-900 dark:text-stone-100">{{ $order->fabric->color }}</span>
                                    </div>
                                    <div>
                                        <span class="text-slate-500 dark:text-stone-400">Kategori:</span>
                                        <span class="font-medium text-slate-900 dark:text-stone-100">{{ $order->fabric->category_label }}</span>
                                    </div>
                                    <div>
                                        <span class="text-slate-500 dark:text-stone-400">Harga:</span>
                                        <span class="font-medium text-slate-900 dark:text-stone-100">Rp {{ number_format((float) $order->fabric->price_per_meter, 0, ',', '.') }}/m</span>
                                    </div>
                                </div>
                            </div>
                        @endif
            ...
            <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-stone-700 dark:bg-stone-800">
                <h2 class="text-lg font-semibold dark:text-stone-100">Timeline Status</h2>
                <div class="mt-4 space-y-3">
                    @forelse ($statusLogs as $log)
                        <div class="rounded-lg border border-slate-200 p-3 dark:border-stone-700">
                            <div class="flex flex-wrap items-center gap-2">
                                <x-status-badge :status="$log->status" />
                                <div class="text-xs text-slate-500 dark:text-stone-400">{{ $log->created_at->format('d M Y H:i') }}</div>
                                <div class="text-xs text-slate-500 dark:text-stone-400">oleh {{ $log->user?->name ?? 'System' }}</div>
                            </div>
                            @if ($log->notes)
                                <div class="mt-2 text-sm text-slate-600 dark:text-stone-300">{{ $log->notes }}</div>
                            @endif
                        </div>
                    @empty
                        <div class="text-sm text-slate-500 dark:text-stone-400">Belum ada riwayat status.</div>
                    @endforelse
                </div>
            </section>

            <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-stone-700 dark:bg-stone-800">
                <h2 class="text-lg font-semibold dark:text-stone-100">Daftar Pembayaran</h2>
                <div class="mt-4 space-y-3">
                    @forelse ($order->payments as $payment)
                        <div class="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-slate-200 p-3 dark:border-stone-700">
                            <div>
                                ...
                                <div class="text-xs text-slate-500 dark:text-stone-400">Metode: {{ strtoupper($payment->payment_method) }}</div>
                            </div>
                            <div class="flex items-center gap-2">
                                ...
                                @if ($payment->proof_file_path)
                                    <a
                                        ...
                                        class="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:border-slate-300 dark:border-stone-600 dark:text-stone-300 dark:hover:border-stone-500"
                                        target="_blank"
                                    >
                                        Lihat Bukti
                                    </a>
                    ...
                    @empty
                        <div class="text-sm text-slate-500 dark:text-stone-400">Belum ada pembayaran.</div>
                    @endforelse
                </div>
            </section>
        </div>
        ...
        <aside class="space-y-6">
            ...
            <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-stone-700 dark:bg-stone-800">
                <h2 class="text-lg font-semibold dark:text-stone-100">Aksi Admin</h2>
                    ...
                    @elseif ($order->status === 'menunggu_dp')
                        ...
                        @else
                            <div class="space-y-4">
                                ...
                                @else
                                    <div class="rounded-xl border border-sky-200 bg-sky-50 p-4 dark:border-sky-900 dark:bg-sky-900/20">
                                        ...
                                        <button type="button" wire:click="openDpForm" class="mt-3 w-full rounded-xl border border-sky-300 px-4 py-2 text-sm font-semibold text-sky-700 hover:bg-sky-100 dark:border-sky-700 dark:text-sky-400 dark:hover:bg-sky-800/30">
                                            Ubah Nominal DP
                                        </button>
                                    </div>
                                @endif
                                ...
                                @if ($showDpForm)
                                    <div class="rounded-xl border border-slate-200 p-4 dark:border-stone-700">
                                        <label class="text-sm font-semibold text-slate-700 dark:text-stone-300">Nominal DP (Rp)</label>
                                        <input type="number" wire:model="dpAmount" class="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm dark:border-stone-600 dark:bg-stone-800 dark:text-stone-100">
                                        @error('dpAmount') <p class="mt-1 text-xs text-rose-600">{{ $message }}</p> @enderror
                                        
                                        <div class="mt-3 flex gap-2">
                                            <button type="button" wire:click="setDpAmount" class="flex-1 rounded-lg bg-slate-900 px-3 py-1.5 text-xs font-semibold text-white hover:bg-slate-800 dark:bg-stone-100 dark:text-stone-900 dark:hover:bg-white">Simpan DP</button>
                                            <button type="button" wire:click="closeDpForm" class="rounded-lg border border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-600 hover:bg-slate-50 dark:border-stone-600 dark:text-stone-300 dark:hover:bg-stone-700">Batal</button>
                                        </div>
                                    </div>
                                @endif
                            </div>
            ...
            @if (!in_array($order->status, ['siap_diambil', 'selesai', 'ditolak', 'dibatalkan']))
                <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-stone-700 dark:bg-stone-800">
                    <div class="flex items-center justify-between">
                        <h2 class="text-lg font-semibold dark:text-stone-100">Edit Total Harga</h2>
                        @if (!$showPriceForm)
                            <button type="button" wire:click="openPriceForm" class="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:border-slate-300 hover:bg-slate-50 dark:border-stone-600 dark:text-stone-300 dark:hover:bg-stone-700">
                                Edit
                            </button>
                        @endif
                    </div>
                    ...
                    <div class="mt-3">
                        <div class="text-sm text-slate-500 dark:text-stone-400">Harga Saat Ini</div>
                        <div class="text-xl font-bold text-slate-900 dark:text-stone-100">
                            Rp {{ number_format((float) $order->estimated_price, 0, ',', '.') }}
                        </div>
                    </div>
            ...
            @if ($order->service->type !== 'vermak')
                <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-stone-700 dark:bg-stone-800">
                    <h2 class="text-lg font-semibold dark:text-stone-100">Preview Desain</h2>
                    <div class="mt-4 space-y-2">
                        @forelse ($order->designFiles as $file)
                            <div class="flex items-center justify-between rounded-lg border border-slate-200 px-3 py-2 dark:border-stone-700">
                                <div class="text-sm text-slate-700 dark:text-stone-300">{{ $file->original_filename }}</div>
                                <button
                                    type="button"
                                    wire:click="previewDesign({{ $file->id }})"
                                    class="rounded-lg border border-slate-200 px-3 py-1 text-xs font-semibold text-slate-700 hover:border-slate-300 dark:border-stone-600 dark:text-stone-300 dark:hover:bg-stone-700"
                                >
                                    Lihat
                                </button>
                            </div>
                        @empty
                            <div class="text-sm text-slate-500 dark:text-stone-400">Belum ada file desain.</div>
                        @endforelse
                    </div>
                </section>
            @endif
    ...
    @if ($showDesignModal)
        <div class="modal-backdrop-enter fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 px-4">
            <div class="modal-content-enter w-full max-w-2xl rounded-2xl bg-white p-5 shadow-xl dark:bg-stone-800">
                <div class="flex items-center justify-between">
