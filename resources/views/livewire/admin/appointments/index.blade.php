<x-slot name="header">
    <div class="flex flex-col">
        <h1 class="text-2xl font-bold text-stone-900 dark:text-stone-100">Jadwal Fitting</h1>
        <p class="text-sm text-stone-500 dark:text-stone-400">Kelola janji temu fitting dengan pelanggan Anda.</p>
    </div>
</x-slot>
...
<div>
    ...
    <div class="lg:col-span-4 xl:col-span-3 space-y-6">
        <!-- Calendar -->
        <div class="rounded-2xl border border-[#ece4d6] bg-white p-5 shadow-sm relative overflow-hidden dark:border-stone-700 dark:bg-stone-800">
            <div class="flex items-center justify-between mb-6">
                <h3 class="font-bold text-stone-900 dark:text-stone-100">{{ $monthName }}</h3>
            ...
            <div class="grid grid-cols-7 gap-1 text-center text-xs font-semibold text-stone-500 dark:text-stone-400 mb-3">
        ...
        <div class="rounded-2xl border border-stone-200 bg-white p-5 shadow-sm dark:border-stone-700 dark:bg-stone-800">
            <h3 class="font-bold text-stone-900 dark:text-stone-100 mb-5">Ringkasan Hari Ini</h3>
            <div class="space-y-5">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3 text-sm font-medium text-stone-700 dark:text-stone-300">
                    ...
                    <span class="font-bold text-xl text-stone-900 dark:text-stone-100">{{ $summary['total'] }}</span>
                </div>
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3 text-sm font-medium text-stone-700 dark:text-stone-300">
                ...
                <div class="flex items-center justify-between">
                    ...
                    <span class="font-bold text-xl text-stone-900 dark:text-stone-100">{{ $summary['selesai'] }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="lg:col-span-8 xl:col-span-9">
        <div class="rounded-2xl border border-stone-200 bg-white shadow-sm overflow-hidden dark:border-stone-700 dark:bg-stone-800">
            <!-- Header section of list -->
            <div class="flex flex-col sm:flex-row sm:items-center justify-between p-6 border-b border-stone-200 bg-[#fdfaf5] dark:border-stone-700 dark:bg-stone-800">
                <div>
                    <h2 class="text-2xl font-bold text-stone-900 dark:text-stone-100">
                        {{ \Carbon\Carbon::parse($dateFilter)->translatedFormat('l, d F Y') }}
                    </h2>
                    <p class="text-sm font-medium text-stone-500 dark:text-stone-400 mt-1">{{ $summary['total'] }} Jadwal Fitting Hari Ini</p>
                </div>
            ...
            <div class="divide-y divide-stone-200 dark:divide-stone-700">
                    ...
                    <div class="flex flex-col sm:flex-row p-6 gap-6 hover:bg-stone-50/50 transition relative">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-start justify-between">
                                <div>
                                    <div class="font-bold text-stone-900 dark:text-stone-100 text-lg truncate">{{ $appointment->customer->name }}</div>
                                    <div class="text-sm font-medium text-stone-500 dark:text-stone-400 mt-1">ID Pesanan: #{{ $appointment->order->order_number }}</div>
                                </div>
                            ...
                            <div class="flex flex-wrap gap-2 mt-4">
                                <span class="inline-flex items-center rounded-md bg-stone-100 px-3 py-1.5 text-xs font-semibold text-stone-600 dark:bg-stone-700 dark:text-stone-300">{{ $appointment->order->service->name }}</span>
                                <span class="inline-flex items-center rounded-md bg-stone-100 px-3 py-1.5 text-xs font-semibold text-stone-600 dark:bg-stone-700 dark:text-stone-300">Fitting</span>
                            </div>
                ...
                @empty
                    <div class="p-16 text-center text-stone-500 dark:text-stone-400 font-medium">
...
<div x-data="{ show: @entangle('showRescheduleModal') }" 
    ...
    <div x-show="show" 
         x-transition
         class="relative w-full max-w-md transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all sm:my-8 border border-stone-100 dark:bg-stone-800 dark:border-stone-700">
        <form wire:submit.prevent="saveReschedule">
            <div class="bg-white dark:bg-stone-800 px-6 pb-6 pt-6">
                <div class="sm:flex sm:items-start">
                    <div class="text-center sm:text-left w-full">
                        <h3 class="text-xl font-bold leading-6 text-stone-900 dark:text-stone-100 mb-2">
                            Jadwal Ulang Fitting
                        </h3>
                        <p class="text-sm text-stone-500 dark:text-stone-400 mb-6">Pilih tanggal dan waktu baru untuk jadwal fitting ini.</p>
                        
                        <div class="space-y-5">
                            <div>
                                <label class="block text-sm font-bold text-stone-700 dark:text-stone-300 mb-1.5">Tanggal Baru</label>
                            ...
                            <div>
                                <label class="block text-sm font-bold text-stone-700 dark:text-stone-300 mb-1.5">Waktu Baru</label>
            ...
            <div class="bg-stone-50/80 dark:bg-stone-700/50 px-6 py-4 sm:flex sm:flex-row-reverse border-t border-stone-100 dark:border-stone-700">
