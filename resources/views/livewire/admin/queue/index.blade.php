<div class="page-enter space-y-6">
    <div>
        ...
        <p class="text-sm text-slate-500 dark:text-stone-400">Urutkan pesanan yang sedang dikerjakan.</p>
    </div>
    ...
    <div class="space-y-6">
        @foreach ($queueGroups as $status => $orders)
            <section class="space-y-3">
                <h2 class="text-lg font-semibold text-slate-800 dark:text-stone-200">{{ $statusLabels[$status] ?? ucfirst($status) }}</h2>
                ...
                @forelse ($orders as $order)
                    ...
                    <div class="hover-lift rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-stone-700 dark:bg-stone-800">
                        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div>
                                <div class="text-base font-semibold text-slate-900 dark:text-stone-100">{{ $order->order_number }}</div>
                                <div class="text-sm text-slate-500 dark:text-stone-400">{{ $order->customer->name }} - {{ $order->service->name }}</div>
                            ...
                            <div class="flex items-center gap-2">
                                <button
                                    ...
                                    class="rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 hover:border-slate-300 dark:border-stone-600 dark:text-stone-300 dark:hover:border-stone-500"
                                >
                                    ▲ Naikkan
                                </button>
                                <button
