<div class="page-enter mx-auto max-w-5xl space-y-6 px-4 pb-20 sm:px-6">
        ...
        @foreach ($statuses as $key => $label)
            <button
                ...
                @class([
                    ...
                    'bg-stone-200 text-stone-500 hover:bg-stone-300 hover:text-stone-700' => $statusFilter !== $key,
                ])
            >
                {{ $label }}
            </button>
    ...
    <div class="space-y-4">
            ...
            function getOrderStatusBadge($status) {
                return match(true) {
                    in_array($status, ['menunggu_appointment', 'menunggu_bahan']) => [
                        'label' => 'Menunggu',
                        'class' => 'bg-amber-50 text-amber-700 ring-1 ring-inset ring-amber-500/30',
                        'dot'   => 'bg-amber-500',
                    ],
                    in_array($status, ['diproses', 'dijahit']) => [
                        'label' => 'Sedang Dijahit',
                        'class' => 'bg-blue-50 text-blue-700 ring-1 ring-inset ring-blue-500/30',
                        'dot'   => 'bg-blue-500',
                    ],
                    $status === 'finishing' => [
                        'label' => 'Finishing',
                        'class' => 'bg-purple-50 text-purple-700 ring-1 ring-inset ring-purple-500/30',
                        'dot'   => 'bg-purple-500',
                    ],
                    $status === 'selesai' => [
                    ...
                    default => [
                        ...
                        'class' => 'bg-stone-50 text-stone-600 ring-1 ring-inset ring-stone-300/50',
                        'dot'   => 'bg-stone-400',
                    ],
                };
            }
        ...
        @forelse ($orders as $order)
            ...
            <div class="group relative overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-stone-200 transition-all duration-300 hover:shadow-md hover:ring-stone-300">
                {{-- Left bar --}}
                ...
                <div class="flex flex-col md:flex-row">
                    ...
                    <a href="{{ route('orders.show', $order) }}" wire:navigate class="flex flex-1 gap-4 py-5 pl-6 pr-5">
                        {{-- Icon --}}
                        <div class="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-lg bg-stone-100 text-stone-500 transition group-hover:bg-stone-200">
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex flex-wrap items-center gap-x-3 gap-y-1">
                                <span class="text-[13px] font-bold text-stone-800">#{{ $order->order_number }}</span>
                                <span class="text-[10px] text-stone-300">•</span>
                                <span class="text-[13px] font-medium text-stone-600">{{ $order->created_at->format('d M Y') }}</span>
                                <span class="text-[10px] text-stone-300">•</span>
                            ...
                            <p class="mt-2 text-[17px] font-bold text-stone-900">{{ $order->service->name }}</p>
                            @if ($order->notes)
                                <p class="mt-0.5 truncate text-sm text-stone-500">{{ \Illuminate\Support\Str::limit($order->notes, 80) }}</p>
                            @endif
                        </div>
                    </a>

                    {{-- Right panel --}}
                    <div class="flex flex-shrink-0 flex-col justify-center border-t border-stone-100 bg-white px-6 py-4 md:w-56 md:border-l md:border-t-0 md:items-end md:text-right">
                        <div class="text-[10px] font-bold uppercase tracking-widest text-stone-400">Total Biaya</div>
                        <div class="mt-0.5 text-lg font-bold {{ $isFinished ? 'text-stone-700' : 'text-[#0044CC]' }}">Rp {{ number_format((float) $order->estimated_price, 0, ',', '.') }}</div>

                        @if ($isFinished)
                            <div class="mt-3 text-[11px] text-stone-400">Diambil Pada</div>
                            <div class="text-xs font-semibold text-stone-700">{{ $order->updated_at->format('d M Y') }}</div>
                            <div class="mt-3 text-right">
                                <a href="{{ route('payments.history.order', $order) }}" wire:navigate class="inline-flex items-center gap-1.5 text-xs font-semibold text-blue-600 transition hover:text-blue-800">
                                    Lihat Nota
                        ...
                        @else
                            <div class="mt-3 text-[11px] text-stone-400">Estimasi Selesai</div>
                            <div class="text-xs font-semibold text-stone-700">{{ $order->estimated_finish_date ? $order->estimated_finish_date->format('d M Y') : '-' }}</div>
                            <div class="mt-3 text-right">
                                <a href="{{ route('orders.show', $order) }}" wire:navigate class="inline-flex items-center gap-1.5 text-xs font-semibold text-blue-600 transition hover:text-blue-800">
                                    Lihat Detail & Lacak
